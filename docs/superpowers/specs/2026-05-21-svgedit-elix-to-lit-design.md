...
- **2026-05-24 (PR-3a LANDED):** PR [#30](https://github.com/bilbospocketses/svgedit/pull/30) squash-merged to master `69ed2cc0` (web-flow signed, all 4 required checks green: build-and-test, Analyze javascript-typescript, Analyze actions, Scorecard analysis). **First of 3 sub-PRs under PR-3.** 4 conversions: `seSpinInput` (244 → 143 LOC, PILOT — native `<input type="number">` + arrows, 3 shadowDOM-piercing sites resolved naturally, `svgEditor.$click` → declarative `@click=`); `seMenu` (125 → 167 LOC — native `<button type="button">` keyboard-accessible trigger, conditional-render popup for firefox layout stability, document-level click + Escape listeners with full disconnectedCallback cleanup); `seMenuItem` (132 → 88 LOC — native `<button role="menuitem">`, single class-field-arrow `_onDocumentKeydown` per convention bullet 8); `seDropdown` (186 → 164 LOC — class rename `Dropdown` → `SeDropdown` per PR-2 cascade, zero external consumers verified, decorative `<input>` + `stepZoom` TODO dropped). 2 deletes: `sePlainMenuButton.ts` (23 LOC, sole registration of `<elix-menu-button>`); `sePlainBorderButton.ts` (34 LOC, transitively dead via sePlainMenuButton). **Cumulative source delta: −238 net LOC** across `src/editor/components/`. **Sub-PR scope decisions locked at planning time** (docs/superpowers/plans/2026-05-24-svgedit-elix-to-lit-pr-3a-plan.md § "Decisions locked"): sub-PR split = 3 sub-PRs (PR-3a / PR-3b / PR-3c) by strategic axis; `<elix-dialog>` replacement = native HTML5 `<dialog>` (PR-3c scope); `sePromptDialog` rename target = `seStatusDialog` (PR-3b scope); `storageDialog` inclusion = yes (PR-3c). **Lessons captured** (worth pasting into PR-3b / PR-3c dispatch packets): (1) Consumer audits for elix-bound rewrites MUST grep elix-internal tags in test files (e.g., `elix-number-spin-box`, `elix-menu-button`), not just `<se-*>` selectors — refines [[feedback_consumer_audit_grep_test_files]] for PR-3 scope. (2) Parallel-batch dispatch has port-8000 contention — vite preview's `--strictPort` + `reuseExistingServer: true` means siblings risk running tests against wrong `dist/`; mitigation = run preview on alternate port (e.g., 8431) with custom playwright config. (3) Firefox-only layout regression observed under parallel-test load — seMenu's always-mounted `position: absolute` popup with `display: none` toggle caused intermittent right-click coordinate drift (paste offset by one SVG viewport); defensive fix = conditional rendering of popup. (4) `<div role="button">` is keyboard-inaccessible (regression on first Lit conversion since elix-menu-button wrapped a real `<button>` internally); always use `<button type="button">` for click triggers. **After PR-3a:** `components/` is mostly elix-free; `PaintBox.ts` + `seColorPicker.ts` remain (jGraduate-bound, PR-4 scope). **PR-3b next:** SePlainAlertDialog + sePromptDialog → seStatusDialog rename + 5 callsite updates.
- **2026-05-24 (PR-3b LANDED):** PR [#35](https://github.com/bilbospocketses/svgedit/pull/35) squash-merged to master `e9e2bea1` (web-flow signed, all 4 required checks green). **Second of 3 sub-PRs under PR-3.** 2 conversions: `SePlainAlertDialog` (92 → 155 LOC — native HTML5 `<dialog>` + slot for textContent + dynamic choice buttons via `map` directive; tag renamed `<se-elix-alert-dialog>` → `<se-plain-alert-dialog>`; class name preserved; full imperative API surface preserved for the 3 `window.seAlert/seConfirm/seSelect` wrappers); `sePromptDialog` → `seStatusDialog` (103 → 110 LOC — file/class/tag renamed; inlined own native `<dialog>` dropping `new SePlainAlertDialog() as any` composition; preserved `close` attribute toggle semantics verbatim per planning lock #4). 5 callsite updates: `dialogs/index.ts`, `EditorStartup.ts`, `ext-opensave.ts` (5 `$id()` sites), `Editor.ts` (comment), `dialogs-extra.spec.js` (both renames). Closes audit input #4 (`sePromptDialog` misnamed → renamed to `seStatusDialog`). **Native HTML5 `<dialog>` pattern established** (first use in svgedit; reusable for all PR-3c dialogs). **After PR-3b:** `dialogs/` mostly elix-free; only 5 elix-dialog-coupled HTML-bound dialogs remain for PR-3c + 3 vendored se-elix overrides + `import 'elix/define/Dialog.js'` in `dialogs/index.ts`. **PR-3c next:** 5 elix-dialog HTML-bound dialogs (svgSourceDialog, imagePropertiesDialog, editorPreferencesDialog, exportDialog, storageDialog) + delete 3 vendored se-elix overrides + drop `import 'elix/define/Dialog.js'`. After PR-3c: ZERO elix imports across `src/`.
- **2026-05-24 (PR-3c LANDED):** PR [#37](https://github.com/bilbospocketses/svgedit/pull/37) squash-merged to master `683cefc6` (web-flow signed, all 4 required checks green). **Third and final sub-PR under PR-3. ZERO elix imports remain; `elix` npm dependency removed.** 5 conversions (native HTML5 `<dialog>` + inline HTML): exportDialog (159 LOC, pilot), svgSourceDialog (230 LOC), imagePropertiesDialog (374 LOC), editorPreferencesDialog (406 LOC), storageDialog (155 LOC). 8 files deleted: 5 HTML templates (content inlined) + 3 vendored se-elix overrides. Infrastructure: `scripts/build-extensions.ts` gains SWC plugin (TC39 decorators in extension bundles); `tests/e2e/helpers.js` updated for native-dialog visibility timing. **Lesson captured:** `@property({ reflect: true }) + updated()` is the ONLY correct pattern for dialog open/close in Lit + native `<dialog>` — manual `attributeChangedCallback` races with Lit's render cycle; `static get observedAttributes()` must include `...super.observedAttributes` to avoid shadowing Lit's property-registered attributes. **PR-3 complete (all 3 sub-PRs shipped).** Item #3 elix -> Lit migration status: PR-1 substrate + references DONE, PR-2 10 pure components DONE, PR-3a/3b/3c DONE. Remaining: PR-4 (jGraduate + jPicker Lit-rewrite, jQuery-based) + PR-5 (jamilih drop + cleanup).
