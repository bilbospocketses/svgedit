...
4. **Color, spacing, and typography come from the `--se-*` design tokens** in `src/editor/styles/tokens.css`. Use the **semantic** role tokens — `--se-bg`, `--se-surface`, `--se-surface-2`, `--se-text`, `--se-text-muted`, `--se-border`, `--se-accent`, `--se-on-accent`, `--se-danger`, `--se-scrim`, `--se-shadow-overlay`, … — plus the scale tokens (`--se-space-*`, `--se-radius-*`, `--se-text-*`, `--se-font-sans`); never the raw primitives (`--se-grey-*`, `--se-teal-*`) and never literal hex. Light/dark is automatic: the semantic tokens are re-pointed under `html[data-theme="dark"]`, so a component built on semantic tokens themes for free. Raw `#hex` is allowed **only** in `tokens.css`; everywhere else the `lint:hex` hex-guard (`npm run lint:hex`) fails the build — mark a genuine functional-color exception with a `hex-guard-allow` comment. _(Updated 2026-06-02 by the M1 design-system migration, which replaced the old `--*-color` names — `--main-bg-color`, `--input-color`, `--orange-color`, etc. — with this token system; the components in `src/editor/dialogs/` are the reference consumers.)_
