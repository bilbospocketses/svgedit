...
import { DocumentIO, type DocumentIOHost } from '../../src/editor/DocumentIO.js'
...
describe('DocumentIO', () => {
  ...
  it('loadSvgString sets the SVG and updates the canvas on success', () => {
    const host = makeHost()
    new DocumentIO(host as unknown as DocumentIOHost).loadSvgString('<svg id="x"/>')
  ...
  it('loadSvgString throws and alerts when setSvgString returns false', () => {
    ...
    expect(() => new DocumentIO(host as unknown as DocumentIOHost).loadSvgString('bad')).toThrow('Error loading SVG')
  ...
  it('loadSvgString with noAlert throws without alerting', () => {
    ...
    expect(() => new DocumentIO(host as unknown as DocumentIOHost).loadSvgString('bad', { noAlert: true })).toThrow('Error loading SVG')
    expect(globalThis.seAlert).not.toHaveBeenCalled()
  })

  it('openPrep returns true when the undo stack is empty (no confirm)', async () => {
    const host = makeHost()
    expect(await new DocumentIO(host as unknown as DocumentIOHost).openPrep()).toBe(true)
  ...
  it('openPrep confirms when there is undo history', async () => {
    ...
    await new DocumentIO(host as unknown as DocumentIOHost).openPrep()
    expect(globalThis.seConfirm).toHaveBeenCalledWith('notification.QwantToOpen')
  })

  it('loadFromString loads via ready()', async () => {
    const host = makeHost()
    await new DocumentIO(host as unknown as DocumentIOHost).loadFromString('<svg/>')
  ...
  it('loadFromDataURI decodes percent-encoded SVG and loads it', async () => {
    const host = makeHost()
    await new DocumentIO(host as unknown as DocumentIOHost).loadFromDataURI('data:image/svg+xml,' + encodeURIComponent('<svg id="d"/>'))
  ...
  it('loadFromURL rejects a cross-origin URL when noAlert is set', async () => {
    const host = makeHost()
    await expect(new DocumentIO(host as unknown as DocumentIOHost).loadFromURL('https://evil.example/x.svg', { noAlert: true }))
